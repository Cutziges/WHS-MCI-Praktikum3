...
public class MaskField extends TextField{
    ...
    private void updateShowingField() {
        ...
        for (int i = 0; i < objectMask.size(); i++) {
            ...
            if (p.isPlainCharacter()) {
                if (textPlain.length() > counterPlainCharInMask) {
                    ...
                    while (!p.isCorrect(c)) {
                        textPlain = textPlain.substring(0, 
                                counterPlainCharInMask) + textPlain.substring(
        ...
        int caretPosition = (textPlain.length() > 0 ? 
                lastPositionPlainCharInMask + 1 
                : firstPlaceholderInMask);
